...
const App: React.FC = () => {
  ...
  // Database State (fetched from server)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDatabase(data.schedules || []);
        setUploadStats(data.stats || { enginesCount: 0, manufCount: 0, enginesFreshmanCount: 0, manufFreshmanCount: 0 });
      }
    } catch (error) {
  ...
  // Upload handler calling the Server API
  const handleDepartmentUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'ENGINES' | 'MANUFACTURING' | 'ENGINES_FRESHMAN' | 'MANUFACTURING_FRESHMAN') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDatabase(result.data.schedules);
        setUploadStats(result.data.stats);
        alert(`تم رفع وتحديث ${result.count} جدول بنجاح. البيانات محفوظة الآن في الخادم.`);
      } else {
      ...
      console.error(error);
      alert('حدث خطأ أثناء رفع الملف. تأكد من صيغة الملف واتصالك بالإنترنت.');
    } finally {
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 text-center relative overflow-hidden border border-gray-100">
              ...
              <div className="mb-8">
                ...
                {isLoading && <p className="text-xs text-tvtc-gold mt-2 animate-pulse">جاري تحميل قاعدة البيانات...</p>}
              </div>
